...
/* Phase 3: Send to large buckets */
  ...
  for (i = 0; i < size; i++){
    ...
    if (flag == 0) { //accommodate the case that all elements in one process fall into one small bucket
      array_counter[i] = N/size;
      printf("Rank %d: local_bucket[%d]'s counter = %d \n",rank,i,array_counter[i]);
    }   
  }
  ...
  for (i = 0; i < size - 1; i++){
    ...
    printf("Rank %d: array_counter[%d] = %d, send displace %d \n",rank,i,array_counter[i],sdisp[i+1]);
  }
